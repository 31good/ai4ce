...
care_dict={10:1,12:1,14:1,16:1,18:1,19:1,23:1,25:1,29:1,30:1,31:1,34:1,35:1,36:1,37:1,38:1,39:1,40:1,41:1}
labels = [
...
def main():
    save_dir="/mnt/hdd_0/KITTI-360/sequences/00"
    ...
    for folder in [label_folder, calib_folder, lidar_folder, pose_folder]:
        if not os.path.isdir(folder):
            os.makedirs(folder)
    """
    pcd_dir = kitti360_root + "/data_3d_semantics/train/2013_05_28_drive_0000_sync/dynamic/*"
    dynamic_lst=sorted(glob.glob(pcd_dir))
    static_lst=sorted(glob.glob(kitti360_root + "/data_3d_semantics/train/2013_05_28_drive_0000_sync/static/*"))
    #pcd_dir = kitti360_root + "/data_3d_semantics/train/2013_05_28_drive_0000_sync/dynamic/0000009886_0000010098.ply"
    box_dir = kitti360_root + "/data_3d_bboxes/train/2013_05_28_drive_0000_sync.xml"
# not sure
    poses_file = "/mnt_0/hdd_0/KITTI-360/data_poses/2013_05_28_drive_0000_sync/cam0_to_world.txt"
    #poses_file = open("/home/allenzj/KITTI-360/data_poses/2013_05_28_drive_0000_sync/cam0_to_world.txt", "r")
    calib_file=open("/mnt_0/hdd_0/KITTI-360/calibration/calib_cam_to_velo.txt","r")
    ...
    for index in range(len(dynamic_lst)):
        ...
        for index in range(len(np_ply_1[:,6])):
            if(np_ply_1[:,6][index] in care_dict):
                care_dict[np_ply_1[:,6][index]]=np_ply_1[:,10][index]
        if(num_verts!=0):
...
#len(np_ply_1[:, 0])
        ...
        #for frame in range(1031):
            ...
            #transform=poses[frame].astype("float32")
